...
class ParcelRanker:
    ...
    def calculate_rankings(self, parcels: gpd.GeoDataFrame, output_dir: str, show_graph: bool = False) -> gpd.GeoDataFrame:
        ...
        Args:
            ...
            output_dir: Directory to save the weights file
            show_graph: Whether to show a graph of the parcel rankings
            
        Returns:
            GeoDataFrame with added ranking columns
        """
        try:
            # Ensure we have a geometry column
            if 'geom' not in parcels.columns and 'geometry' not in parcels.columns:
                raise ValueError("No geometry column found in parcels DataFrame")
            
            # Set the active geometry column
            geom_col = 'geom' if 'geom' in parcels.columns else 'geometry'
            parcels = parcels.set_geometry(geom_col)
            
            # Calculate distances to features
            parcels['dist_to_airport'] = parcels.geometry.distance(airport_point)
            parcels['dist_to_transmission'] = parcels.geometry.distance(transmission_line)
